...
/**
 * Generic sublabels generated by form frameworks that describe the input type
 * rather than the question being asked (e.g. Google Forms "Date" sublabel under "Joining Date").
 */
...
export function resolveAccessibleLabel(
  ...
  if (labelledBy) {
    ...
    if (textParts.length > 0) {
      ...
      // If the label is generic (like "Date" in Google Forms), prefer question heading from container
      if (container) {
        const heading = resolveHeadingText(container);
        if (heading && !isGenericSublabel(heading)) {
          return { label: heading, ariaLabel, placeholder };
        }
      }
  ...
  if (ariaLabel) {
    ...
    if (cleaned && !isGenericSublabel(cleaned)) {
      return { label: cleaned, ariaLabel, placeholder };
    }
    if (container) {
      const heading = resolveHeadingText(container);
      if (heading && !isGenericSublabel(heading)) {
    ...
    if (cleaned) return { label: cleaned, ariaLabel, placeholder };
  }
